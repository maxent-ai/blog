...
### [A Multi-Objective Learning to re-Rank Approach to Optimize Online Marketplaces for Multiple Stakeholders.](https://arxiv.org/pdf/1708.00651.pdf)
...
$\\og(m/p)$ = profit margin define as percentage here.
...
### [Joint Optimization of Profit and Relevance for Recommendation Systems in E-commerce](http://ceur-ws.org/Vol-2440/short1.pdf)
...
This is a blog plot from LinkedIn. Here, they are building a LinkedIn notifications recommendation system, which notifies members about various activities within their network. To build an efficient notification system, they are optimizing for the following objects:

1. CTR
2. Number of sessions (In a general word, let the user visit/open app)
...
The overall objective function looks as below:

$$P_click + \alpha * \delta P_visit > \gamma$$

where,

1. $p_click$ is the probability of click by member
2. ΔpVisit can be define as (p(Visit | send ) - p(Visit | not send)). It's the difference between sending a notification now vs not sending a notification.
3. a is the hyperparameter that measures the relative importance of the two utilities.
4. 𝛾 is the threshold applied.

Finding an optimal value of x={a,𝛾} can maximize the sessions without compromising on CTR and send volume. To ensure the overall performance, they used c1 and c2 constants as contained for CTR(x), and Send Volume(x).
...
Using the above contained, they define a single objective function as below:

$$\max_x U(x) = f_{session}(x) + \lambda(1 \{f_{CTR}(x) > c_1 \} + 1\{f_{sent-volume(x)} < c_2\}),$$

By converting a MOO into SOO using constrained-based optimization, they learnt global hyperparameters (i.e., a single value) for all members. 
Based on online A/B experiments, they found that learning separate hyperparameters for cohorts of members gives better results than converting a MOO into SOO.
